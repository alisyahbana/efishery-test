...
type MysqlUserStatement struct {
	...
	GetUserByPhoneAndPasswordQuery *sqlx.Stmt
...
func init() {
	...
	stmt.GetUserByPhoneAndPasswordQuery = database.Prepare(database.GetDBMaster(), GetUserByPhoneAndPasswordQuery)
...
func (m MysqlUserData) GetUserByPhoneAndPassword(phone string, password string) (*User, error) {
	var user User
	err := stmt.GetUserByPhoneAndPasswordQuery.Get(
		&user,
		phone,
		password,
	)
